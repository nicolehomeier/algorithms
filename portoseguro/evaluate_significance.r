...
#upsample to balance the classes
  ...
  #pfit = normGini(as.numeric(as.character(testing$target)),predfit[,2])
    ...
    gbm_testing_kfold = gbm_testing_kfold + predgbm[,2]
    med_testing_kfold = med_testing_kfold + mpred
...
gbm_testing_kfold = gbm_testing_kfold/kfold
med_testing_kfold = med_testing_kfold/kfold
...
#order the testing sample and hold-back sample by the model predictions
ord_gbm = testing$target[order(gbm_testing_kfold,decreasing=T)]
ord_med = testing$target[order(med_testing_kfold,decreasing=T)]
...
names(out) = c("GBM-5fold","Median-5fold")
...
#get the risk groups
sect = seq(1,length(gbm_testing_kfold)+10,by=round(length(gbm_testing_kfold)/cats,digits=0))
act_sect = seq(1,nrow(hb_test)+10,by=round(nrow(hb_test)/cats,digits=0))
for (i in 1:cats){
    losses_gbm[i] = length(which(ord_gbm[sect[i]:sect[i+1]]==1))*cc
    losses_med[i] = length(which(ord_med[sect[i]:sect[i+1]]==1))*cc
    actual_losses_gbm[i] = length(which(ord_gbm_hb[act_sect[i]:act_sect[i+1]]==1))*cc
    actual_losses_med[i] = length(which(ord_med_hb[act_sect[i]:act_sect[i+1]]==1))*cc
}
prem_gbm = round(losses_gbm/(sect[2]-1) + gain,digits=2)
prem_med = round(losses_med/(sect[2]-1) + gain,digits=2)
print(prem_gbm)
print(prem_med)
print(actual_losses_gbm/(prem_gbm*(act_sect[2]-1)))
print(actual_losses_med/(prem_med*(act_sect[2]-1)))
